...
const NewsTimeline: React.FC<NewsTimelineProps> = ({ news, showHeader: _showHeader = false }) => {
  ...
  const typeColors: Record<string, { bg: string, fg: string, icon: string }> = {
    ...
    news: {
      bg: isDark ? 'rgba(191, 97, 106, 0.18)' : 'rgba(191, 97, 106, 0.12)',
      fg: tc.error,
      icon: 'FaBolt'
    },
    talk: {
  ...
  const getDescriptionLength = useCallback((description: string) => {
    ...
    const baseText = description.length > maxLength
      ? truncateText(description, maxLength).replace(/\.\.\.$/, '')
      : description;
  ...
  const getRowDescription = useCallback((item: NewsItem) => {
    if (item.type.toLowerCase() === 'publication' && item.detailItems?.length) {
      return item.detailItems[0].title
    }
    return item.description
  }, [])

  const getMemoryDumpBadge = useCallback((item: NewsItem) => {
    const title = item.title.toLowerCase();

    if (title.includes('paper accepted')) {
      const match = item.title.match(/^(\d+)\s+paper(s)?\s+accepted/i);
      if (match) {
  ...
  const getExpandedTypeLabel = useCallback((item: NewsItem) => {
    if (item.type.toLowerCase() === 'release') {
      return 'Release';
    }
    return item.type.toLowerCase() === 'publication' ? 'Publication' : 'News';
  }, [])

  const renderExpandedDescription = useCallback((item: NewsItem) => {
    if (item.title === 'Started MPhil study at HKUST StatML Lab') {
      return (
        <>
          Began MPhil study at{' '}
          <Box as="span" color={termCommand} fontWeight="medium">
            HKUST StatML Lab
          </Box>{' '}
          ...
          <Link
            ...
            color={termCommand}
            fontWeight="medium"
            _hover={{ color: termHighlight, textDecoration: 'underline' }}
          >
            Prof. Tong Zhang
          </Link>
          .
    ...
    return highlightData(item.description, { num: termHighlight, kw: termCommand, str: termSuccess });
  }, [termCommand, termHighlight, termSuccess])
      ...
      >
        ...
        <Flex align="center" gap={1} flex={["1 1 100%", "1 1 auto"]}>
          ...
          <Text isTruncated>
            ...
            <Box as="span" color={termHighlight} display={["none", "inline"]}>'news'</Box>
            <Box as="span" color={termSecondary} display={["none", "inline"]}>)</Box>
          </Text>
        </Flex>
      ...
      >
        ...
        <Flex align="center" gap={0} overflow="hidden">
          ...
          <Flex align="center" gap={1} px={1.5} flexShrink={0} display={["none", "flex"]}>
            ...
            <Text color={termCommand}>~/news</Text>
          </Flex>
        </Flex>
            ...
            >
                ...
                <Box flex="1">
                  <Flex align="center" gap={1}>
                    ...
                    {item.secondaryBadge && (
                      <Text
                        ...
                        px={[0.5, 1]}
                        borderRadius="sm"
                        bg={isDark ? 'rgba(235, 203, 139, 0.16)' : 'rgba(196, 125, 70, 0.12)'}
                        color={termWarning}
                        fontWeight="bold"
                        whiteSpace="nowrap"
                      >
                  ...
                  {showDescription && (
                    <Text color={termSecondary} fontSize={["3xs", "2xs"]} isTruncated mt={0.5}>
                      {getDescriptionLength(getRowDescription(item))}
                    </Text>
                  )}
                </Box>
              ...
              <Collapse in={expandedItems[index]} animateOpacity>
                ...
                >
                  {!item.detailItems?.length && (
                    <>
                      <Flex align="start" gap={2} mb={2}>
                        ...
                        <Box flex={1} minW={0}>
                          ...
                          <Flex fontSize={["3xs", "2xs"]} gap={[1, 2]} flexWrap="wrap" align="center">
                            <Text
                              color={item.type.toLowerCase() === 'publication' ? termSuccess : typeColors[item.type.toLowerCase()]?.fg || typeColors.default.fg}
                              fontWeight="bold"
                              textTransform="uppercase"
                            >
                              {item.type.toLowerCase() === 'publication' ? 'Poster' : item.badge}
                            </Text>
                            <Text color={termSecondary}>·</Text>
                            ...
                            {item.secondaryBadge && (
                              <>
                                <Text color={termSecondary}>·</Text>
                                <Text color={termWarning} fontWeight="medium">
                                  {item.secondaryBadge}
                                </Text>
                              </>
                            )}
                      ...
                      {item.description && (
                        ...
                        >
                          {renderExpandedDescription(item)}
                        </Text>
                      )}
                    </>
                  )}
                  {item.detailItems && item.detailItems.length > 0 && (
                    <Flex direction="column" gap={2} mt={item.description ? 1 : 0}>
                      {item.detailItems.map((detail, detailIndex) => (
                        <Flex
                          key={`${item.title}-${detailIndex}`}
                          align="start"
                          gap={3}
                          py={1.5}
                          borderTop={detailIndex === 0 ? '1px dotted' : 'none'}
                          borderColor={termBorder}
                        >
                          <Box
                            p={1.5}
                            borderRadius="md"
                            bg={isDark ? 'rgba(136,192,208,0.12)' : 'rgba(42,118,156,0.08)'}
                            display={["none", "flex"]}
                            alignItems="center"
                            justifyContent="center"
                            flexShrink={0}
                            mt={0.5}
                          >
                            <DynamicIcon name="FaFilePdf" boxSize={[3, 4]} color={termCommand} />
                          </Box>
                          <Box flex={1} minW={0}>
                            <Text fontSize={["2xs", "xs"]} fontWeight="bold" color={termText} lineHeight="1.5" mb={1}>
                              {detail.title}
                            </Text>
                            <Flex fontSize={["3xs", "2xs"]} gap={[1, 2]} flexWrap="wrap" align="center" mb={1.5}>
                              <Text
                                color={item.type.toLowerCase() === 'release' ? termSuccess : termCommand}
                                fontWeight="bold"
                                textTransform="uppercase"
                              >
                                {getExpandedTypeLabel(item)}
                              </Text>
                              <Text color={termSecondary}>·</Text>
                              <Text color={termWarning}>{item.date}</Text>
                              <Text color={termSecondary}>·</Text>
                              <Text color={termSuccess} fontWeight="medium">
                                {item.secondaryBadge ?? (item.badge.toLowerCase().includes('technical report')
                                  ? 'Preprint'
                                  : (item.type.toLowerCase() === 'publication' ? 'Poster' : item.badge))}
                              </Text>
                            </Flex>
                            {detail.summary && (
                              <Text fontSize={["2xs", "xs"]} color={tc.secondary} lineHeight="1.6" mb={2}>
                                <Box as="span" color={termCommand} fontWeight="medium">
                                  {detail.shortLabel ?? detail.title.split(':')[0]}
                                </Box>
                                <Box as="span" color={termSecondary}>: </Box>
                                {detail.summary}
                              </Text>
                  ...
                  {item.links.length > 0 && !(item.detailItems && item.detailItems.length > 0) && (
