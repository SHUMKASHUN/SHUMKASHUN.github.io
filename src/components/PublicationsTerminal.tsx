...
import { FaChartBar, FaVideo, FaProjectDiagram, FaFileAlt, FaAtom, FaStar, FaRobot, FaGlobe, FaHandRock, FaCloudSun, FaFutbol, FaFilePdf } from 'react-icons/fa'
...
interface PublicationsTerminalProps {
  embedded?: boolean
}

const PublicationsTerminal: React.FC<PublicationsTerminalProps> = ({ embedded = false }) => {
  ...
  const filteredPublications = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      ...
      const monthA = a.month ? monthOrder[a.month.toLowerCase()] || 0 : 0
      const monthB = b.month ? monthOrder[b.month.toLowerCase()] || 0 : 0
  ...
  return (
    <Box w="full" minH={embedded ? 'auto' : '100vh'} bg={embedded ? 'transparent' : useColorModeValue('gray.50', 'gray.900')} py={embedded ? 0 : 8}>
      <VStack spacing={embedded ? 0 : 6} maxW={embedded ? 'full' : '1400px'} mx="auto" px={embedded ? 0 : [2, 4, 6]}>
        {/* Terminal Container */}
        <Box
          w="full"
          borderRadius="md"
                        ...
                        <Tooltip label="Paper">
                          <Link href={pub.links.paper} isExternal onClick={(e) => e.stopPropagation()}>
                            <Badge colorScheme="blue" fontSize="2xs">
                              <HStack spacing={1}>
                                <Icon as={FaFilePdf} boxSize="10px" />
                                <Text as="span">PAPER</Text>
                              </HStack>
                            </Badge>
                          </Link>
                        </Tooltip>
                          ...
                          <Flex flexWrap="wrap" gap={2}>
                            {Object.entries(pub.links).map(([key, url]) => url && (
                              <Link key={key} href={url} isExternal>
                                ...
                                >
                                  <HStack spacing={1}>
                                    {(key === 'paper' || key === 'arxiv') && <Icon as={FaFilePdf} boxSize="10px" />}
                                    <Text as="span">{key.replace(/([A-Z])/g, ' $1').trim()}</Text>
                                  </HStack>
                                </Badge>
                              </Link>
                            ))}
